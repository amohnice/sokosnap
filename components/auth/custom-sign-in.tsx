...
export function CustomSignIn() {
  ...
  const [verification, setVerification] = useState<{ active: boolean, type: "first" | "second" | null, strategy: string | null }>({ active: false, type: null, strategy: null })
    ...
    try {
      ...
      if (result.status === "complete") {
        ...
        if (emailFactor) {
          ...
          setVerification({ active: true, type: "first", strategy: "email_code" });
        ...
        if (totpFactor) {
          setVerification({ active: true, type: "second", strategy: "totp" });
          ...
          setVerification({ active: true, type: "second", strategy: "phone_code" });
          ...
          setVerification({ active: true, type: "second", strategy: "email_code" });
          toast.success("2FA code sent to your email!");
        } else {
          const strategies = result.supportedSecondFactors?.map(f => f.strategy).join(", ");
          toast.error(`2FA required. Available methods: ${strategies || "None"}`);
        }
      } else {
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      let result;
      if (verification.type === "first") {
        result = await clerk.client.signIn.attemptFirstFactor({
          strategy: verification.strategy as any,
          code,
        })
      } else {
        result = await clerk.client.signIn.attemptSecondFactor({
          strategy: verification.strategy as any,
          code,
        })
      }
          ...
          <div className="text-center space-y-2 mb-4">
            {verification.type === "first" ? (
              <p className="text-sm font-medium text-white/70">
                Your email requires verification. We sent a new code to<br/>
                <span className="text-white font-bold">{emailAddress}</span>
              </p>
            ) : (
              <p className="text-sm font-medium text-white/70">
                Two-step verification is required.<br/>
                <span className="text-white font-bold">Please enter your 2FA code below.</span>
              </p>
            )}
          </div>
          <form onSubmit={handleVerify} className="space-y-4">
            <div className="space-y-1.5">
              <Label htmlFor="code" className="text-white/70 font-semibold mb-1 block">6-Digit Code</Label>
              <Input 
                id="code" 
                value={code} 
                onChange={(e) => setCode(e.target.value)} 
                className="bg-white/5 border-white/10 text-white placeholder:text-white/30 h-14 tracking-[0.4em] text-center font-mono text-2xl focus-visible:ring-primary rounded-xl"
                placeholder="000000"
                maxLength={6}
                required 
              />
            ...
            <Button
              type="button"
              variant="ghost"
              onClick={() => setVerification({ active: false, type: null, strategy: null })}
              className="w-full text-white/50 hover:text-white font-medium"
            >
              Cancel
            </Button>
          </form>
